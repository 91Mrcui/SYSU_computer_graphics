...
namespace TinyRenderer
{
	...
	void TRWindowsApp::processEvent()
	{
		...
		while (SDL_PollEvent(&m_events) != 0)
		{
			...
			{
				/*static bool firstWheel = true;
				if (firstWheel)
				{
					firstWheel = false;
					m_last_wheel_pos = m_events.wheel.y;
					m_wheel_delta = 0;
				}
				else
				{
					m_wheel_delta = m_events.wheel.y - m_last_wheel_pos;
					m_last_wheel_pos = m_events.wheel.y;
				}*/
	...
	double TRWindowsApp::updateScreenSurface(unsigned char *pixels, int width, int height, int channel)
	{
		//Update pixels
		...
		//FPS counting
		{
			...
			if (m_fps_time_recorder > 1000.0)
			{
				...
				std::stringstream ss;
				ss << m_fps;
				SDL_SetWindowTitle(m_window_handle, (m_window_title + " FPS: " + ss.str()).c_str());
			}
		}
