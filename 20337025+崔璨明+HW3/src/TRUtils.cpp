...
{
	glm::mat4 TRUtils::calcViewPortMatrix(int width, int height)
	{
	...
	glm::mat4 TRUtils::calcViewMatrix(glm::vec3 camera, glm::vec3 target, glm::vec3 worldUp)
	{
	...
	glm::mat4 TRUtils::calcPerspProjectMatrix(float fovy, float aspect, float near, float far)
	{
	...
	glm::mat4 TRUtils::calcOrthoProjectMatrix(float left, float right, float bottom, float top, float near, float far)
	{
